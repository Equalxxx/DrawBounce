...
public class StringTableReader : EditorWindow
{
	...
	private string selectedExcelPath = "Assets/JoyconFrameWork/4_LocalizeManager/Table/StringTable.xlsx";
            ...
            if (itemDataTable.Count > 0)
            {
                for (int i = 0; i < itemDataTable.Count; i++)
                {
                    ...
                    for (int column = 1; column <= itemDataTable[i].NumberOfColumns; column++)
                    {
                        ...
                        if (Convert.ToString(itemDataTable[i].GetValue(1, column)) == "Font")
                            fontNameColumn = column;
                    }
